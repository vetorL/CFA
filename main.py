    ...
    def start(self):
        ...
        try:
            ...
            while True:
                ...
                if (
                    self.dados.getAverage()["luminosidade"] > 90
                    and self.dados.getAverage()["uv"] > 5
                ):
                ...
                if self.UV_sensor.getUVIntensity() > 6:
                    self.display.run_uv_very_high()
                elif self.UV_sensor.getUVIntensity() > 5:
                    self.display.run_uv_high()
                elif self.UV_sensor.getUVIntensity() > 4:
                    self.display.run_uv_moderate()
                elif self.UV_sensor.getUVIntensity() > 3:
                    self.display.run_uv_low()
